...
SampleCloudComponent::SampleCloudComponent(TagDatabaseManager &db,
  ...
  openGLContext.setContinuousRepainting(
      true); // Enable continuous repainting for twinkling animation
...
void SampleCloudComponent::setItems(const std::vector<MediaItem> &items) {
  ...
  layoutThread =
      std::thread([this, itemsCopy]() { runLayoutAsync(itemsCopy); });
}
...
void SampleCloudComponent::runLayoutAsync(std::vector<MediaItem> items) {
  ...
  juce::MessageManager::callAsync([safeThis, newNodes = std::move(newNodes),
    ...
    if (auto *comp = safeThis.getComponent()) {
      ...
      // Render a large glowing planet at the center of each cluster for the
      // label
...
SampleCloudComponent::calculateClusterLayoutInternal(
  ...
  std::sort(outClusters.begin(), outClusters.end(),
            [](const TagCluster &a, const TagCluster &b) {
      ...
      for (size_t i = 0; i < tagCount; ++i) {
        for (size_t j = i + 1; j < tagCount; ++j) {
          ...
          float dist = std::sqrt(delta.x * delta.x + delta.y * delta.y +
                                 delta.z * delta.z);
          float r1 = std::max(45.0f, 30.0f + std::sqrt(static_cast<float>(
                                                 outClusters[i].count)) *
                                                 14.0f);
          float r2 = std::max(45.0f, 30.0f + std::sqrt(static_cast<float>(
                                                 outClusters[j].count)) *
                                                 14.0f);
  ...
  for (auto &node : nodesCopy) {
    ...
    float maxCloudRadius = std::max(
        55.0f, 30.0f + std::sqrt(static_cast<float>(totalInTag)) * 16.0f);
    float scatterFactor =
        std::pow(static_cast<float>(nodeHash % 1000) / 1000.0f, 0.65f);
    ...
    node.targetPos = {cPos.x + rDist * xDir, cPos.y + rDist * yDir,
                      cPos.z + rDist * zDir};
    node.currentPos = node.targetPos;
  }

  // Edges calculation removed for O(N^2) performance reasons, as OpenGL doesn't
  // render CPU edges anymore
...
void SampleCloudComponent::mouseWheelMove(
  ...
  if (e.y > getHeight() - 40 && e.x < getWidth() - 320) {
    ...
    zoomScale =
        juce::jlimit(0.3f, 4.0f, zoomScale * (1.0f + wheel.deltaY * 2.0f));
    repaint();
  }
}
...
void SampleCloudComponent::newOpenGLContextCreated() {
  ...
  juce::String vertexShader =
      ...
      "    float phase = position.x * 0.1 + position.y * 0.15 + position.z * "
      "0.05;\n"
      "    float noise = rand(position.xy * 0.01) * 0.5;\n"
      "    float twinkle = 0.65 + 0.35 * sin(time * (1.5 + noise) + phase) * "
      "cos(time * 0.8 + phase * 1.3);\n"
...
// Since we don't have glew, we can use the JUCE OpenGLHelpers or just rely on
// the driver But glBlendFunc, glEnable, glDrawArrays are standard OpenGL 1.1
// available in opengl32.dll
#if !defined(_WIN32)
...
#define GL_PROGRAM_POINT_SIZE 0x8642
  ...
  auto projectionMatrix = juce::Matrix3D<float>::fromFrustum(
      -aspect * 0.1f, aspect * 0.1f, -0.1f, 0.1f, 0.2f, 10000.0f);

  juce::Matrix3D<float> viewMatrix;
  viewMatrix = viewMatrix *
               juce::Matrix3D<float>::fromTranslation(
                   {-panOffset.x, panOffset.y, -cameraDistance / zoomScale});
