...
class ReassignedSpectrum
{
  ...
  public:
    ...
    double
    reassignedPhase(long idx, double, double) const
    {
        return reassignedPhase(idx);
    }
    double
    reassignedMagnitude(double, long intBinNumber) const
    {
  ...
  private:
    ...
    //  (time-derivative) window in the real part and the time-ramp window in
    //  the imagnary part, and the other having the unmodified window in the
    //  real part and, if computing mixed deriviatives, the time-ramp
    //  time-derivative window in the imaginary part.
    //
