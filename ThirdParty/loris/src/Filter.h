...
class Filter
{
  public:
    ...
    //! \param fbackbegin is the beginning of a sequence of feedback
    //! coefficients
    ...
    //! construction, and may add functionality, but for efficiency, the
    //! filtering operation is non-virtual.
    ~Filter(void);
    ...
    m_gain(gain)
{
    if (*fbackbegin == 0.)
    {
        Throw(InvalidObject, "Tried to create a Filter with feeback "
                             "coefficient at zero delay equal to 0.0");
    }
    ...
    if (*fbackbegin != 1.)
    {
        ...
        std::transform(m_ffwdcoefs.begin(), m_ffwdcoefs.end(),
                       m_ffwdcoefs.begin(), [a0](double c) { return c / a0; });
        std::transform(m_fbackcoefs.begin(), m_fbackcoefs.end(),
                       m_fbackcoefs.begin(), [a0](double c) { return c / a0; });
        m_fbackcoefs[0] = 1.;
    }
}
