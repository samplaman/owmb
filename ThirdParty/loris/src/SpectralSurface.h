...
class SpectralSurface
{
  ...
  private:
    ...
    std::vector<Partial>
        mPartials;         //! the Partials comprising the surface are
    ...
    mMaxSurfaceAmp(0.0)
{
    ...
    {
        Throw(InvalidArgument,
              "The SpectralSurface is zero amplitude everywhere!");
    }
...
template <typename Iter>
void
SpectralSurface::scaleAmplitudes(Iter b, Iter e)
{
    while (b != e)
    {
        scaleAmplitudes(*b);
...
template <typename Iter>
void
SpectralSurface::setAmplitudes(Iter b, Iter e)
{
    while (b != e)
    {
        setAmplitudes(*b);
