...
class FundamentalEstimator
{
  public:
    ...
    static constexpr int DefaultAmpFloor =
        -60; //! the default absolute amplitude threshold in dB

    static constexpr int DefaultAmpRange =
        30; //! the default floating amplitude threshold in dB

    static constexpr int DefaultFreqCeiling =
        4000; //! the default frequency threshold in Hz

    static constexpr int DefaultPrecisionOver100 =
        10; //! the default frequency precision in 1/100 Hz

    static constexpr int DefaultMinConfidencePct =
...
class FundamentalFromSamples : public FundamentalEstimator
{
  ...
  public:
    ...
    //!         threshold are added to the envelope
    LinearEnvelope
    buildEnvelope(const double *sampsBeg, unsigned long nsamps,
                  double sampleRate, double tbeg, double tend, double interval,
                  double lowerFreqBound, double upperFreqBound,
                  double confidenceThreshold)
    {
        return buildEnvelope(sampsBeg, sampsBeg + nsamps, sampleRate, tbeg,
    ...
    //!         threshold are added to the envelope
    LinearEnvelope
    buildEnvelope(const std::vector<double> &samps, double sampleRate,
                  double tbeg, double tend, double interval,
                  double lowerFreqBound, double upperFreqBound,
                  double confidenceThreshold)
    {
        return buildEnvelope(&samps[0], &samps[0] + samps.size(), sampleRate,
                             tbeg, tend, interval, lowerFreqBound,
                             upperFreqBound, confidenceThreshold);
    }
    ...
    value_type
    estimateAt(const double *sampsBeg, unsigned long nsamps, double sampleRate,
    ...
    value_type
    estimateAt(const std::vector<double> &samps, double sampleRate, double time,
               double lowerFreqBound, double upperFreqBound)
    {
...
class FundamentalFromPartials : public FundamentalEstimator
{
  ...
  public:
    ...
    //! \param precisionHz is the precision in Hz with which the
    //! fundamental estimates will be made.
    FundamentalFromPartials(double precisionHz = DefaultPrecisionOver100 *
                                                 0.01);
    ...
    //!         threshold are added to the envelope
    LinearEnvelope
    buildEnvelope(const PartialList &partials, double tbeg, double tend,
                  double interval, double lowerFreqBound, double upperFreqBound,
                  double confidenceThreshold)
    {
        return buildEnvelope(partials.begin(), partials.end(), tbeg, tend,
    ...
    //!         F0Estimate.h)
    value_type
    estimateAt(const PartialList &partials, double time, double lowerFreqBound,
               double upperFreqBound)
    {
        return estimateAt(partials.begin(), partials.end(), time,
                          lowerFreqBound, upperFreqBound);
    }
