...
class Morpher
{
    ...
    double _minBreakpointGapSec; //! the minimum time gap between two
                                 //! Breakpoints in the morphed Partials.
                                 //! Morphing two Partials can generate a third
                                 //! Partial having Breakpoints arbitrarily
                                 //! close together in time, and this makes
                                 //! morphs huge. Raising this threshold limits
                                 //! the Breakpoint density in the morphed
  ...
  public:
    ...
    Partial
    morphPartial(Partial src, Partial tgt, int assignLabel)
    {
    ...
    //!         to evaluate the morphing functions and tgtPartial).
    //! \return the morphed Breakpoint
    Breakpoint morphSrcBreakpoint(const Breakpoint &bp,
    ...
    //!         to evaluate the morphing functions and tgtPartial).
    //! \return the morphed Breakpoint
    Breakpoint morphTgtBreakpoint(const Breakpoint &bp,
    ...
    //! Default is true.
    void
    enableLogAmpMorphing(bool enable = true)
    {
        _doLogAmpMorphing = enable;
    }

    //!	Enable (or disable) log-domain frequency morphing.
    //! Default is false.
    void
    enableLogFreqMorphing(bool enable = true)
    {
  ...
  private:
    ...
    double interpolateAmplitude(double srcAmp, double tgtAmp,
                                double alpha) const;
    ...
    double interpolateFrequency(double srcFreq, double tgtFreq,
                                double alpha) const;
    double interpolatePhase(double srcphase, double tgtphase,
                            double alpha) const;
